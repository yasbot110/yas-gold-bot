# -*- coding: utf-8 -*-
"""Mashhad channel parsers — usable prototype for the project."""
import urllib.request, re, ssl, html as H

ZWNJ='\u200c'; TSEP='\u066c'  # zero-width non-joiner and Arabic thousands separator
...
# --- MARKIZ_ARG parser (Mashhad melted gold) ---
...
def parse_markiz(msgs):
    ...
    for mid,tm,t in msgs:  # oldest to newest; newest wins
        m=RE_BUY.search(t)
        if m: buy=to_int(m.group(1)); ts_buy=tm; last_id=max(last_id,mid)
        m=RE_SELL.search(t)
