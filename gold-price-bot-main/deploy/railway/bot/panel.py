...
def main_menu() -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton(
            f"➕ اضافه به فروش: {cfg.get('sell_markup'):,} تومان",
            callback_data="ask:sell_markup",
        )],
        [InlineKeyboardButton(
...
def settings_text() -> str:
    ...
    lines = [
        ...
        f"➕ اضافه به فروش: <b>{cfg.get('sell_markup'):,} تومان</b> "
        "(فروش نمایشی = خام + این عدد)",
        f"➖ کسر خرید: <b>{cfg.get('buy_deduction'):,} تومان</b> "
        "(خرید = فروش نمایشی − این عدد)",
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data in ("ask:channel", "ask:deduction", "ask:sell_markup", "ask:addadmin", "ask:deladmin"):
        await query.answer()
        prompts = {
            ...
            "ask:deduction": (
                ...
                "مثلاً: 100000 یعنی خرید = فروش نمایشی − ۱۰۰ هزار تومان",
                "wait:deduction",
            ),
            "ask:sell_markup": (
                "مبلغ اضافه به فروش را به **تومان** بفرست (فقط عدد، بدون جداکننده).\n"
                "مثلاً: 50000 یعنی فروش نمایشی = فروش خام + ۵۰ هزار تومان.\n"
                "برای خاموش کردن 0 بفرست.",
                "wait:sell_markup",
            ),
...
async def handle_wait_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == "wait:channel":
        ...
        await update.effective_message.reply_text(
            ...
            "از این به بعد: قیمت خرید = فروش نمایشی − همین عدد."
        )
    elif state == "wait:sell_markup":
        # Sell markup — admin adds an amount to the raw sell price before publishing.
        # 0 means "off" (use the raw channel price unchanged). Negative values are
        # not allowed (use the deduction to lower the buy side instead).
        cleaned = text.replace(",", "").replace("٬", "").strip()
        if not cleaned.isdigit():
            await update.effective_message.reply_text(
                "❌ فقط عدد مثبت به تومان بفرست؛ مثلاً: 50000 یا 0 برای خاموش"
            )
            context.user_data["await_input"] = "wait:sell_markup"  # keep waiting
            return
        value = int(cleaned)
        cfg.update("sell_markup", value)
        if value == 0:
            await update.effective_message.reply_text(
                "✅ اضافه به فروش خاموش شد؛ فروش نمایشی = فروش خام کانال مرجع."
            )
        else:
            await update.effective_message.reply_text(
                f"✅ اضافه به فروش روی {value:,} تومان ثبت شد.\n"
                f"از این به بعد: فروش نمایشی = فروش خام + {value:,} تومان؛ "
                f"خرید = فروش نمایشی − کسر خرید."
            )
