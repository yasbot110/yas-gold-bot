...
# ── End-of-day closing message (20:01 Tehran) ────────────────────────────
# The closing text lives in formatting.py (closing_text) so all message
# templates are colocated and easy to find/edit. This handler is the only
# orchestrator: it checks the schedule and sends the message.


async def publish_closing_message(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send the end-of-day closing message to the publish channel at 20:01
    Tehran (one minute after the last 1-min price post, ensuring all price
    updates have flushed first). Runs every minute; only fires when
    minute == 1 AND hour == WORK_END_MIN's hour. Uses 'last_closing_date' in
    DB to deduplicate so a restart in the same day doesn't double-post."""
    if not settings_mod.get("auto_enabled"):
        return
    target = settings_mod.get("channel")
    if not target:
    ...
    now_tehran = datetime.now(TEHRAN)
    end_h = WORK_END_MIN // 60
    # Fire at HH:01 (one minute after the end of the work window) so any last
    # in-window price update (HH:00) gets published first.
    if now_tehran.hour != end_h or now_tehran.minute != 1:
        return

    # De-dupe: one closing message per day (key = YYYY-MM-DD Tehran)
    day_key = now_tehran.strftime("%Y-%m-%d")
    state = db.get_state()
    if state.get("last_closing_date") == day_key:
        return  # already sent today

    try:
        await context.bot.send_message(
            chat_id=target,
            text=fmt.closing_text(),
            parse_mode=ParseMode.HTML,
        )
        db.save_state("last_closing_date", day_key)
        log.info("closing message sent to %s for %s", target, day_key)
    except Exception as exc:  # noqa: BLE001
        log.error("closing message to %s failed: %s", target, exc)
...
async def cmd_testclosing(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Test the end-of-day closing message manually."""
    if not is_admin(update):
        return
    target = settings_mod.get("channel")
    if not target:
    ...
    try:
        ...
        await update.effective_message.reply_text("✅ پیام پایان روز تستی ارسال شد.")
    except Exception as exc:  # noqa: BLE001
        await update.effective_message.reply_text(
            f"❌ ارسال نشد: <code>{html_mod.escape(str(exc))}</code>",
            parse_mode=ParseMode.HTML,
...
async def post_init(app: Application) -> None:
    ...
    # End-of-day closing message job — runs every minute, sends only at 20:00
    app.job_queue.run_repeating(
        publish_closing_message,
        interval=60,
        first=15,
        name="closing-eod",
    )
    log.info("closing-eod job scheduled (runs every minute, sends at %02d:01)", WORK_END_MIN // 60)
...
def build_app(token: str) -> Application:
    ...
    app.add_handler(CommandHandler("testclosing", cmd_testclosing))
